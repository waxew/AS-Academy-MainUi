...
fun AcademySearchScreen(
    ...
    LazyColumn(
        ...
        item {
            Text("جست‌وجو", style = MaterialTheme.typography.headlineMedium)
        }
        item {
            OutlinedTextField(
                value = query,
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            if (result.body.isNotBlank()) {
                Text(result.body.take(220))
            }
            Text("نوع: ${result.refType}", style = MaterialTheme.typography.bodySmall)
            Button(onClick = { onOpen(result.refId, result.refType) }) {
                Text("باز کردن")
            }
        }
    }
            ...
            items(sorted, key = { it.achievementId }) { achievement ->
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(
                        ...
                        Text(
                            titleForAchievement(achievement.achievementId),
